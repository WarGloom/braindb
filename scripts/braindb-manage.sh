...
compose() {
  ...
  if [[ -f "$ENV_FILE" ]] && has_compose_profile internal-db; then
    args+=(--profile internal-db)
  fi
...
ensure_database_url() {
  ...
  if has_compose_profile internal-db; then
    return 0
  fi
  die ".env must set DATABASE_URL, or set COMPOSE_PROFILES=internal-db for the bundled Postgres"
}
...
fetch_codex_models() {
  require_cmd curl
  local root host_root payload

  root="$(openai_compatible_root_url)"
  host_root="$(host_probe_url "$root" || true)"

  payload="$({ fetch_url "$root/codex/v1/models" || { [[ -n "$host_root" ]] && fetch_url "$host_root/codex/v1/models"; } || true; })"
  [[ -n "$payload" ]] || return 1
...
for model in models:
    ...
    if not model:
        continue
    model = model.removeprefix('openai/').removeprefix('codex/')
...
warn_if_unconfigured() {
  local database_url llm_profile deepinfra_key nim_key openai_key agent_model agent_base_url agent_use_responses
  ...
  agent_use_responses="$(env_value AGENT_USE_RESPONSES false)"
  ...
  case "$database_url" in
    postgresql://user:password@host:5432/braindb)
      ...
      ;;
    "")
      has_compose_profile internal-db || warn "DATABASE_URL is empty and COMPOSE_PROFILES does not include internal-db"
      ;;
  esac
  ...
  case "$llm_profile" in
    ...
    openai_compatible|local_ollama)
      ...
      if [[ "$agent_model" == openai/codex/* && "$agent_use_responses" != "true" ]]; then
        env_set AGENT_USE_RESPONSES true
        log "set AGENT_USE_RESPONSES=true for $agent_model"
      fi
      ;;
  esac
}
...
logs_stack() {
  ...
  for service in "$@"; do
    case "$service" in
      db|postgres|database) services+=(braindb_db) ;;
      *) services+=("$service") ;;
    esac
  done
  if [[ "${FOLLOW_LOGS:-true}" == "true" ]]; then
    compose logs -f --tail="${TAIL_LINES:-200}" "${services[@]}"
  else
    compose logs --tail="${TAIL_LINES:-200}" "${services[@]}"
  fi
}
...
Env overrides:
  DOCKER_BIN=docker|podman  Docker-compatible CLI to use (default: podman when present)
  TAIL_LINES=200            Lines shown by logs
  FOLLOW_LOGS=true|false    Follow logs or print once
EOF
}
